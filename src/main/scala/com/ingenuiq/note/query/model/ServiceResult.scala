package com.ingenuiq.note.query.model

/**
  * Represents a result from a call to a service actor, being either a FullResult (Some), EmptyResult (None) or a Failure.
  * Has the full complete of methods like map, flatMap and filter to treat as a Monad
  */
...
/**
  ...
  */
sealed case class FailureResult(failType: FailureType.Value, message: ErrorMessage, exception: Option[Throwable] = None) extends Empty {
  type A = Nothing
